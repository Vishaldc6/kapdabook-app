...
export const getDatabaseStats = async () => {
  try {
    ...
    const [totalRevenue] = await db.getAllAsync('SELECT SUM(meter * price_rate) as total FROM Bill WHERE payment_received = 1');
    const [pendingAmount] = await db.getAllAsync('SELECT SUM(meter * price_rate) as total FROM Bill WHERE payment_received = 0');
